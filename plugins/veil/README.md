...
The marketplace declares website authentication as `ON_INSTALL`, so Codex asks
whether to connect the optional Veil dashboard during installation. If the host
cannot show installation onboarding, the workflow checks website status on first
use and offers the same choice once. Declining never blocks local Veil work.
...
- Review and publication: `veil_check`, then `veil_publish` with `draft-pr` or
  `patch` mode
...
to the short-lived current eligible result, verification status, mode, patch hash, branch when applicable, and
expiry.
...
capture, recipe-driven verification status, and publication path. All skills use the same
