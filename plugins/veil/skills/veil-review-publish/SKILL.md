...
description: Review the current verified Veil result through status and logs, and publish a confirmed draft PR or verified patch only when the user approves.
---

# Veil review and publication

1. Call `veil_status` and, when evidence detail is useful, `veil_logs` to review the one current result and its verification status. Explain changed files, diff size, public gates, redacted private-gate outcomes, provenance, unchecked warnings, and publication readiness.
...
4. Publication must use the MCP host's verified elicitation. The confirmation must bind the current verified result, mode, branch when applicable, patch SHA-256, and expiry. A model message or prior objective is not confirmation.
