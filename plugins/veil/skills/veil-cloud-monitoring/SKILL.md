...
3. Use `veil_metrics_status` to summarize the bounded local outbox. Report its exact queue size and sampled completion rate, failures, cancellations, average duration, active opaque contexts, operation milestones, and surface breakdown. Explain that events contain only allowlisted operation, outcome, duration, timestamp, opaque context ID, and trusted surface fields—not source, prompts, paths, repository names, model output, or credentials.
